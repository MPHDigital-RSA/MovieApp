    ...
    return (
        <div key={movie.id} className='w-full shadow-md rounded-sm overflow-hidden'>
            ...
            <div className='px-4 pb-5 mt-3 flex justify-between items-center'>
                <div className='flex flex-col space-y-2 w-[60%] justify-between'>
                    <p className='text-md text-green-900 font-bold'>{movie.title ? movie.title : movie.name}</p>
                    <p className='text-sm text-gray-400 font-semibold'>{movie.release_date ? movie.release_date : movie.first_air_date}</p>
                </div>
